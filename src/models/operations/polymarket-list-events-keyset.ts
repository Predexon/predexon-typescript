/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v4-mini";
import { remap as remap$ } from "../../lib/primitives.js";
import * as data from "../data/index.js";

export const PolymarketListEventsKeysetServerList = [
  "https://api.predexon.com",
] as const;

export type PolymarketListEventsKeysetRequest = {
  /**
   * Filter by status: open or closed
   */
  status?: data.EventStatusOption | null | undefined;
  /**
   * Filter by category (e.g. Sports, Crypto)
   */
  category?: string | null | undefined;
  /**
   * Search in title (minimum 3 characters)
   */
  search?: string | null | undefined;
  /**
   * Filter by event ID(s)
   */
  id?: Array<string> | null | undefined;
  /**
   * Filter by event slug(s)
   */
  slug?: Array<string> | null | undefined;
  /**
   * Filter by tag slug(s)
   */
  tag?: Array<string> | null | undefined;
  /**
   * Sort by
   */
  sort?: data.EventSortOption | undefined;
  /**
   * Number of events to return
   */
  limit?: number | undefined;
  /**
   * Cursor for the next page
   */
  paginationKey?: string | null | undefined;
};

/** @internal */
export type PolymarketListEventsKeysetRequest$Outbound = {
  status?: string | null | undefined;
  category?: string | null | undefined;
  search?: string | null | undefined;
  id?: Array<string> | null | undefined;
  slug?: Array<string> | null | undefined;
  tag?: Array<string> | null | undefined;
  sort?: string | undefined;
  limit: number;
  pagination_key?: string | null | undefined;
};

/** @internal */
export const PolymarketListEventsKeysetRequest$outboundSchema: z.ZodMiniType<
  PolymarketListEventsKeysetRequest$Outbound,
  PolymarketListEventsKeysetRequest
> = z.pipe(
  z.object({
    status: z.optional(z.nullable(data.EventStatusOption$outboundSchema)),
    category: z.optional(z.nullable(z.string())),
    search: z.optional(z.nullable(z.string())),
    id: z.optional(z.nullable(z.array(z.string()))),
    slug: z.optional(z.nullable(z.array(z.string()))),
    tag: z.optional(z.nullable(z.array(z.string()))),
    sort: z.optional(data.EventSortOption$outboundSchema),
    limit: z._default(z.int(), 20),
    paginationKey: z.optional(z.nullable(z.string())),
  }),
  z.transform((v) => {
    return remap$(v, {
      paginationKey: "pagination_key",
    });
  }),
);

export function polymarketListEventsKeysetRequestToJSON(
  polymarketListEventsKeysetRequest: PolymarketListEventsKeysetRequest,
): string {
  return JSON.stringify(
    PolymarketListEventsKeysetRequest$outboundSchema.parse(
      polymarketListEventsKeysetRequest,
    ),
  );
}
