...
export class ClientSDK {
  ...
  constructor(options: SDKOptions) {
    ...
    if (!this.#logger && env().PREDEXON_DEBUG) {
      this.#logger = console;
    }
  }
...
// redact sensitive headers before debug logging
const __SENSITIVE_HEADERS = new Set([
  "x-api-key",
  "authorization",
  "cookie",
  "set-cookie",
...
function __redactHeader(k: string, v: string): string {
  return __SENSITIVE_HEADERS.has(k.toLowerCase())
...
async function logRequest(logger: Logger | undefined, req: Request) {
  ...
  for (const [k, v] of req.headers.entries()) {
    logger.log(`${k}: ${__redactHeader(k, v)}`);
  }
  logger.groupEnd();
