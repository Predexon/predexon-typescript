...
import { PredexonDefaultError } from "../models/errors/predexon-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | PredexonDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | PredexonDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new PredexonDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new PredexonDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
